...
const ConvMenuItem2ItemType = (arr:Array<MenuItem>):Array<any> =>{
  ...
  arr.forEach((item)=>{
    ...
    itemTypeList.push({
          ...
          children: (!item.children||item.children.length===0)
                      ?undefined:ConvMenuItem2ItemType(item.children),
    })}
  )
...
function SideMenu({cstore}:{cstore:TodoStore}){
  ...
  useEffect(() => {
    ...
    axios.get('http://127.0.0.1:8000/rights?_embed=children')
      .then(res=>{
